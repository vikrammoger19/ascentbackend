...
class Entities(models.Model):
    entity_id = models.CharField(max_length=10, primary_key=True, unique=True)
    ...
    def save(self, *args, **kwargs):
        if not self.entity_id:
            # Get the last entity record and extract the numeric part of the ID
            last_entity = Entities.objects.all().order_by('entity_id').last()
            if last_entity:
                last_id = int(last_entity.entity_id[6:])  # Extract numeric part after 'ENTITY'
                new_id = last_id + 1
            else:
                new_id = 1
            # Format the new ID as ENTITY0001, ENTITY0002, etc.
            self.entity_id = f'ENTITY{new_id:04d}'
